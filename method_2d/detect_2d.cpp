...
struct Candidate { float cx,cy,cz; int n; float score; };

int main(int argc, char** argv) {
    if (argc < 2) { std::cerr<<"Usage: "<<argv[0]<<" <scene> <out>\n"; return 1; }
    std::string scene_path=argv[1], out=argv[2];
    mkdir(out.c_str(),0755);

    // Read config
    float voxel_sz=0, cluster_tol=0.10, gm=0.1, rh=4.0; int gp=25;
    ...
    for(auto&idx:clusters){
        ...
        Candidate c={ct[0],ct[1],ct[2],n,s};
